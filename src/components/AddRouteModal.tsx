...
export function AddRouteModal({ isOpen, onClose, onCreated }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      present({ message: `Route "${res.item.name}" created`, duration: 2500, position: 'top', color: 'success' })
      onCreated()
      handleClose()
    } catch (err: any) {
      const msg = err?.response?.data?.message || 'Failed to create route'
      setError(msg)
