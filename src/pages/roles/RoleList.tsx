...
export default function RoleList() {
  ...
  const { data, isLoading, isError, isFetching } = useRoleList(search, currentPage, perPage)
  const deleteRole = useDeleteRole()

  const rows = useMemo(() => data?.items ?? data?.roles ?? [], [data])
  ...
  const columns: Column<Role>[] = [
    {
      ...
      render: r => <p className="font-medium text-slate-900 dark:text-slate-100">{r.name}</p>,
    },
    { key: 'description', header: 'Description' },
    {
      key: 'permissions', header: 'Permissions', align: 'right',
      render: r => (
        <span className="font-mono text-xs text-slate-500">{countPermissions(r.permissions)}</span>
      ),
    },
    { key: 'usersCount', header: 'Users', align: 'right' },
    {
      key: 'status', header: 'Status', align: 'center',
      render: r => <Badge variant={r.status === 1 ? 'success' : 'neutral'}>{r.status === 1 ? 'Active' : 'Inactive'}</Badge>,
    },
    {
      key: 'actions', header: '', align: 'right', width: '90px',
      render: r => (
        <div className="flex items-center gap-1 justify-end">
          <button onClick={e => { e.stopPropagation(); navigate(`/roles/view/${r.uuid}`) }}
            className="p-1.5 rounded-lg text-slate-400 hover:text-sky-600 hover:bg-sky-50 transition-colors"><Eye className="w-4 h-4" /></button>
          <button onClick={e => { e.stopPropagation(); navigate(`/roles/edit/${r.uuid}`) }}
            className="p-1.5 rounded-lg text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-colors"><Pencil className="w-4 h-4" /></button>
          <button onClick={e => { e.stopPropagation(); if (confirm('Delete role?')) deleteRole.mutate(r.uuid) }}
            className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 className="w-4 h-4" /></button>
        </div>
      ),
    },
  ]
  ...
  return (
    <PageLayout>
      ...
      <div className="flex items-center justify-between mb-5 gap-4">
        <p className="text-sm text-slate-500">{isLoading ? 'Loading…' : `${total} roles`}</p>
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 h-9 w-64 px-3 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 focus-within:border-brand-400 transition-all">
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setCurrentPage(1) }}
              placeholder="Search roles…" className="flex-1 bg-transparent text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none" />
          </div>
          <Button icon={<Plus className="w-4 h-4" />} onClick={() => navigate('/roles/add')}>
            Add Role
          </Button>
        </div>
      </div>
      ...
      {/* Table */}
      {isError && <p className="text-sm text-red-500 mb-3">Failed to load roles.</p>}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700/60 overflow-hidden">
        <DataTable columns={columns} data={rows} keyField="uuid"
          loading={isLoading}
          onRowClick={r => navigate(`/roles/view/${r.uuid}`)}
