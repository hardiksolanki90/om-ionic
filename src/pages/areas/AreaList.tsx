...
import { useAreaList, useDeleteArea } from '../../hooks/useAreas'
...
import { Area, isAreaActive } from '../../types/area'
import { parseUrlParams, buildUrlParams } from '../../lib/paginationUtils'
import { PaginationFooter } from '../../components/ui/PaginationFooter'

export default function AreaList() {
  const importExport = useImportExport('areas')
  ...
  const { data, isLoading, isError, isFetching } = useAreaList(search, currentPage, perPage)
  const deleteArea = useDeleteArea()

  const rows = useMemo(() => data?.items ?? data?.areas ?? [], [data])
  const total = data?.total ?? 0
  const lastPage = data?.lastPage ?? 1

  const handleDelete = (area: Area) => {
    if (confirm('Delete area?')) deleteArea.mutate(area.uuid)
  }

  const columns: Column<Area>[] = [
    {
      key: 'code', header: 'Code', sortable: true,
      render: r => <span className="font-mono text-sm font-medium text-slate-700 dark:text-slate-300">{r.code}</span>,
    },
    {
    ...
    },
    {
      ...
      render: r => (
        <Badge variant={isAreaActive(r.status) ? 'success' : 'neutral'}>
          {isAreaActive(r.status) ? 'Active' : 'Inactive'}
        </Badge>
      ),
    },
    {
      ...
      render: r => (
        <div className="flex items-center gap-1 justify-end">
          <button
            onClick={e => { e.stopPropagation(); navigate(`/areas/view/${r.uuid}`) }}
            className="p-1.5 rounded-lg text-slate-400 hover:text-brand-600 hover:bg-brand-50 transition-colors"
          ><Eye className="w-4 h-4" /></button>
          <button
            onClick={e => { e.stopPropagation(); navigate(`/areas/edit/${r.uuid}`) }}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700/60 overflow-hidden">
        {isError ? (
          <div className="py-16 text-center text-sm text-red-500">
            Failed to load areas. Check your connection.
          </div>
        ) : (
          <DataTable
            columns={columns}
            ...
            onRowClick={r => navigate(`/areas/view/${r.uuid}`)}
          />
        )}
