...
import { useAreaViewDetails, useAreaPerformance } from '../../hooks/useAreas'

import { isAreaActive } from '../../types/area'

function formatStatus(status: string | boolean | undefined): string {
  return isAreaActive(status) ? 'Active' : 'Inactive'
}
...
export default function AreaView() {
  ...
  const { data: viewData, isLoading, isError } = useAreaViewDetails(id!)
  ...
  } = useAreaPerformance(id!, range, performanceEnabled)

  const area = viewData?.data?.area
  const overview = viewData?.data?.overview
  const performance = performanceData?.data
  const chartData = performance?.chartData ?? []
  const topRoutes = viewData?.data?.topRoutes ?? []
  ...
  const tabs: { id: TabId; label: string; icon: React.ReactNode }[] = [
    { id: 'details',       label: 'Details',       icon: <MapPin className="w-4 h-4" /> },
    ...
    return (
      <PageLayout>
        <div className="flex items-center justify-center h-64">
          <p className="text-slate-500">Loading area details...</p>
        </div>
      </PageLayout>
    )
  }
    ...
    return (
      <PageLayout>
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          <p className="text-slate-500">Area not found or failed to load.</p>
          <Button variant="outline" onClick={() => navigate('/areas')}>Back to Areas</Button>
        </div>
      </PageLayout>
    )
  }
        ...
        <div className="flex items-start justify-between mb-6 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-900 dark:text-white">{area.name}</h1>
                <p className="text-xs font-mono text-slate-400 mt-0.5">{area.code}</p>
                <Badge variant={isAreaActive(area.status) ? 'success' : 'neutral'} className="mt-2">
                  {formatStatus(area.status)}
                </Badge>
              </div>
            </div>
          </div>
          <Button variant="outline" icon={<Pencil className="w-4 h-4" />} onClick={() => navigate(`/areas/edit/${area.uuid}`)}>Edit</Button>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-6">
                <h3 className="text-base font-bold text-slate-900 dark:text-white mb-3">About Area</h3>
                <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                  Geographic sales territory covering {overview?.totalRoutes ?? 0} routes and{' '}
                  {overview?.totalCustomers ?? 0} customers across your distribution network.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  {field('Area Code', area.code, <Hash className="w-4 h-4" />)}
                  {field('Area Name', area.name, <MapPin className="w-4 h-4" />)}
                  {field('Status', formatStatus(area.status), <MapPin className="w-4 h-4" />)}
                  {area.createdAt &&
                    field(
                      'Created',
                  ...
                  {area.updatedAt &&
                    field(
                      'Last Updated',
                      new Date(area.updatedAt).toLocaleDateString(undefined, {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                      }),
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-6">
                <h3 className="text-base font-bold text-slate-900 dark:text-white mb-5 flex items-center gap-2">
                  <RouteIcon className="w-5 h-5 text-slate-400" /> Top Routes in Area
                </h3>
                {topRoutes.length > 0 ? (
                  <div className="space-y-3">
                    {topRoutes.map((route, idx) => (
                      <button
                        key={route.uuid}
                        type="button"
                        onClick={() => navigate(`/routes/view/${route.uuid}`)}
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-xs text-slate-500 font-mono">{route.code}</span>
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-xs text-slate-500">{route.orders} orders</p>
                        </div>
                      </button>
                    ))}
                  </div>
                ) : (
                  <p className="text-sm text-slate-500 italic">No route sales data yet.</p>
                )}
              </div>

              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-6">
                <h3 className="text-base font-bold text-slate-900 dark:text-white mb-5 flex items-center gap-2">
                  <Users className="w-5 h-5 text-slate-400" /> Top Customers in Area
                </h3>
                {topCustomers && topCustomers.length > 0 ? (
                  <div className="space-y-3">
                    {topCustomers.map((customer, idx) => (
                      <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/40">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-brand-100 text-brand-600 flex items-center justify-center font-bold text-xs">{idx + 1}</div>
                          <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{customer.name}</span>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-bold text-slate-900 dark:text-white">${Number(customer.revenue).toLocaleString()}</p>
                  ...
                  <p className="text-sm text-slate-500 italic">No customer data available.</p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-brand-600 rounded-2xl p-6 text-white shadow-lg shadow-brand-500/20">
                ...
                <p className="text-4xl font-bold">
                  ${Number(overview?.totalLifetimeRevenue ?? 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
                <div className="mt-4 pt-4 border-t border-brand-500/30 space-y-3">
                  ...
                  <div>
                    <p className="text-brand-100 text-xs">Routes</p>
                    <p className="text-xl font-bold mt-0.5">
                      {overview?.activeRoutes ?? 0} / {overview?.totalRoutes ?? 0} active
                    </p>
                  </div>
                </div>
              </div>
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-5 space-y-4">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-sky-50 dark:bg-sky-900/30 text-sky-600"><ShoppingCart className="w-5 h-5" /></div>
                  <div>
                    <p className="text-xs text-slate-500">Orders Delivered</p>
                    <p className="text-xl font-bold text-slate-900 dark:text-white">{overview?.totalOrdersDelivered ?? 0}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-violet-50 dark:bg-violet-900/30 text-violet-600"><RouteIcon className="w-5 h-5" /></div>
                  <div>
                    <p className="text-xs text-slate-500">Active Routes</p>
                    <p className="text-xl font-bold text-slate-900 dark:text-white">{overview?.activeRoutes ?? 0}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6 pb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                { label: 'Revenue', value: `$${summary.totalRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 })}`, icon: <DollarSign className="w-5 h-5" />, color: 'emerald' },
                { label: 'Orders', value: summary.totalOrders, icon: <ShoppingCart className="w-5 h-5" />, color: 'violet' },
              ].map(s => (
                <div key={s.label} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-5 flex items-center gap-4">
              ...
              <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-700/60">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    {area.name} · {performance?.rangeLabel ?? (range === 'day' ? 'Today' : range === 'week' ? 'This Week' : 'This Month')}
                  </p>
                </div>
