...
import { useRouteViewDetails, useRoutePerformance } from '../../hooks/useRoutes'
...
export default function RouteView() {
  ...
  const { data: viewData, isLoading, isError } = useRouteViewDetails(id!)
  ...
  } = useRoutePerformance(id!, range, performanceEnabled)

  const route = viewData?.data?.route
    ...
    return (
      <PageLayout>
        <div className="flex items-center justify-center h-64">
          <p className="text-slate-500">Loading route details...</p>
        </div>
      </PageLayout>
    )
  }
    ...
    return (
      <PageLayout>
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          <p className="text-slate-500">Route not found or failed to load.</p>
          <Button variant="outline" onClick={() => navigate('/routes')}>Back to Routes</Button>
        </div>
      </PageLayout>
    )
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs font-mono text-slate-400 mt-0.5">{route.code}</p>
              </div>
            </div>
          </div>
          <Button variant="outline" icon={<Pencil className="w-4 h-4" />} onClick={() => navigate(`/routes/edit/${route.uuid}`)}>Edit</Button>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-6">
                <h3 className="text-base font-bold text-slate-900 dark:text-white mb-3">About Route</h3>
                <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                  {'No description provided.'}
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  {field('Route Code', route.code, <MapPin className="w-4 h-4" />)}
                  {field('Area', route.areaName || 'None', <MapPin className="w-4 h-4" />)}
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-6">
                <h3 className="text-base font-bold text-slate-900 dark:text-white mb-5 flex items-center gap-2">
                  <Users className="w-5 h-5 text-slate-400" /> Top Customers on Route
                </h3>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-5 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-slate-500">Total Orders Delivered</p>
              ...
              <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-700/60">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    {route.name} · {performance?.rangeLabel ?? (range === 'day' ? 'Today' : range === 'week' ? 'This Week' : 'This Month')}
                  </p>
                </div>
