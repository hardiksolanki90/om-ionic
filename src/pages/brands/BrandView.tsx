...
import { useBrandViewDetails, useBrandPerformance } from '../../hooks/useBrands'
...
export default function BrandView() {
  ...
  const { data: viewData, isLoading, isError } = useBrandViewDetails(id!)
  ...
  } = useBrandPerformance(id!, range, performanceEnabled)

  const brand = viewData?.data?.brand
    ...
    return (
      <PageLayout>
        <div className="flex items-center justify-center h-64">
          <p className="text-slate-500">Loading brand details...</p>
        </div>
      </PageLayout>
    )
  }
    ...
    return (
      <PageLayout>
        <div className="flex flex-col items-center justify-center h-64 gap-4">
          <p className="text-slate-500">Brand not found or failed to load.</p>
          <Button variant="outline" onClick={() => navigate('/brands')}>Back to Brands</Button>
        </div>
      </PageLayout>
    )
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs font-mono text-slate-400 mt-0.5">{brand.code}</p>
                <Badge variant={isActive(brand.status) ? 'success' : 'neutral'} className="mt-2">
                  {formatStatus(brand.status)}
                </Badge>
              </div>
            </div>
          </div>
          <Button variant="outline" icon={<Pencil className="w-4 h-4" />} onClick={() => navigate(`/brands/edit/${brand.uuid}`)}>
            Edit
          </Button>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-6">
            <div className="lg:col-span-2 space-y-6">
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700/60 shadow-sm p-6">
                <h3 className="text-base font-bold text-slate-900 dark:text-white mb-3">About Brand</h3>
                <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                  Product brand with {overview?.totalItems ?? 0} items ({overview?.activeItems ?? 0} active) in your catalog.
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                  {field('Brand Code', brand.code, <Hash className="w-4 h-4" />)}
                  {field('Brand Name', brand.name, <Tag className="w-4 h-4" />)}
                  {field('Status', formatStatus(brand.status), <Tag className="w-4 h-4" />)}
                  {brand.createdAt &&
                    field(
                      'Created',
                  ...
                  {brand.updatedAt &&
                    field(
                      'Last Updated',
                      new Date(brand.updatedAt).toLocaleDateString(undefined, {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric',
                      }),
                  ...
                  <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-700/60">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        {brand.name} · {performance?.rangeLabel ?? (range === 'day' ? 'Today' : range === 'week' ? 'This Week' : 'This Month')}
                      </p>
                    </div>
