...
const computeOrderTotals = (items: OrderItem[]): OrderTotals => {
  let grossTotal = 0
  let totalDiscount = 0
  let totalTax = 0
  let totalQty = 0
...
const toOrderPayload = (form: OrderFormType): OrderFormType => ({
  ...form,
...
export default function OrderForm() {
  ...
  useEffect(() => {
    if (isEdit && data?.data) {
      ...
      setForm({
        ...
        salesmanId: String(o.salesmanId ?? ''),
        routeId: String((o as any).routeId ?? ''),
      ...
      setCustomerName(o.customer ? `${o.customer.firstName} ${o.customer.lastName}` : '')
      setRouteName((o as any).routeName ?? '')
      setSalesmanName(o.salesman ? `${o.salesman.firstName} ${o.salesman.lastName}` : '')
    }
  }, [data, isEdit])
  ...
  if (isEdit && isLoading) {
    return (
      <PageLayout noScroll>
        <div className="flex items-center justify-center h-full text-slate-500">Loading Order...</div>
      </PageLayout>
    )
  }
