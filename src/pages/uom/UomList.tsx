...
import { useUomList, useCreateUom, useUpdateUom, useDeleteUom } from '../../hooks/useUoms'
...
export default function UomList() {
  const importExport = useImportExport('uoms')
  ...
  const { data, isLoading, isError, isFetching } = useUomList(search, currentPage, perPage)
  const deleteUom = useDeleteUom()

  const rows = useMemo(() => data?.items ?? data?.uoms ?? [], [data])
  const total = data?.total ?? 0
  const lastPage = data?.lastPage ?? 1

  const handleDelete = (uom: Uom) => {
    if (confirm('Delete UOM?')) deleteUom.mutate(uom.uuid)
  }
    ...
    { key: 'status', header: 'Status', align: 'center', render: r => <Badge variant={r.status === 'active' ? 'success' : 'neutral'}>{r.status === 'active' ? 'Active' : 'Inactive'}</Badge> },
    {
      ...
      render: r => (
        <div className="flex items-center gap-1 justify-end">
          <button onClick={e => { e.stopPropagation(); navigate(`/uom/view/${r.uuid}`) }} className="p-1.5 rounded-lg text-slate-400 hover:text-sky-600 hover:bg-sky-50 transition-colors"><Eye className="w-4 h-4" /></button>
          <button onClick={e => { e.stopPropagation(); navigate(`/uom/edit/${r.uuid}`) }} className="p-1.5 rounded-lg text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-colors"><Pencil className="w-4 h-4" /></button>
  ...
  return (
    <PageLayout>
      <div className="flex items-center justify-between mb-5 gap-4">
        <p className="text-sm text-slate-500">{isLoading ? 'Loading…' : `${total} UOMs`}</p>
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 h-9 w-64 px-3 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 focus-within:border-brand-400 transition-all">
            ...
            <input value={search} onChange={e => { setSearch(e.target.value); setCurrentPage(1) }} placeholder="Search UOMs…" className="flex-1 bg-transparent text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none" />
          </div>
          <ImportExportButtons
          ...
          />
          <Button icon={<Plus className="w-4 h-4" />} onClick={() => navigate('/uom/add')}>Add UOM</Button>
        </div>
      </div>
      {isError && <p className="text-sm text-red-500 mb-3">Failed to load UOMs.</p>}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700/60 overflow-hidden">
        <DataTable columns={columns} data={rows} keyField="uuid" loading={isLoading} emptyMessage="No UOMs found." />
